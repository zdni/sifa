...
class Prescription_model extends MY_Model
{
  ...
  function __construct()
  {
    ...
    parent::set_join_key('prescription_id');
  }
   ...
   * @author madukubah
   */
  public function prescriptions($start = 0, $limit = NULL, $status = NULL)
  {
    if (isset($limit)) {
      $this->limit($limit);
    }
    ...
    $this->select('patient.name AS patient_name');
    $this->join(
      'users',
      'users.id = prescription.user_id',
      'inner'
    );
    $this->join(
      'patient',
